...
if options:
    ...
    with col2:
        ...
        # Convert video frames to uint8 format and rescale pixel values
        video_uint8 = [((frame - tf.reduce_min(frame)) / (tf.reduce_max(frame) -
                        tf.reduce_min(frame)) * 255).numpy().astype(np.uint8) for frame in video]
